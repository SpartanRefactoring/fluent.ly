...
public enum file {
  ...
  private static boolean exists(final @Nullable String path) {
    if (path == null)
      return false;
  ...
  @NotNull public static String read(final @NotNull File f) throws IOException {
    final @NotNull String ls = de.fault(System.getProperty("line.separator")).to("/");
  ...
  @NotNull static String read(final @NotNull Path ¢) throws IOException {
